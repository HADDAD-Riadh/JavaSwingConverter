...
public class treeHeritageMicroConcept extends JPanel {
		...
		for (int i = 0; i < classes.size(); i++)
		{

			{
				...
				for (int m = 0; m < properties.size(); m++)
				{
					...
					{
						classeName = classes.get(i).getUri().substring(classes.get(i).getUri().indexOf("#") + 1);

						for (int l = 0; l < properties.get(m).getDomains().size(); l++)
							if (properties.get(m).getDomains().get(l).toString().endsWith(classeName))
							{
	...
	protected static ImageIcon createImageIcon(String path)
	{
		...
		java.net.URL imgURL = treeHeritageMicroConcept.class.getResource(path);
		if (imgURL != null)
		{
